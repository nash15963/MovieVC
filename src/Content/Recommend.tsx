...
const Recommend = () => {
  ...
  useEffect(() => {
    ...
    const HandleFetchMovie = async () => {
      const randomPageNumber = getRandomInt(1, 450);

      if (randomPageNumber > 0) {
        const Url = `https://api.themoviedb.org/3/movie/top_rated?api_key=7dcd0053886fc8ec5f0fc4ee25fa55a1&language=zh-TW&page=${randomPageNumber}`;
        await fetch(Url)
          .then((res) => res.json())
          .then((data) => {
            if (!ignore) {
              const results: randomMovie[] = data.results.map((ele: any) => {
                if (ele.poster_path) {
                  return {
                    id: ele.id,
  ...
  return <RecommendComponent dataObj={dataObj} />;
};
